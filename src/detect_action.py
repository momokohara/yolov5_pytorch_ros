...
from yolov5_pytorch_ros.msg import BoundingBoxesAction
from yolov5_pytorch_ros.msg import BoundingBoxesResult
from yolov5_pytorch_ros.msg import BoundingBoxesFeedback
...
#            self.detected_objects_topic, BoundingBoxes, queue_size=10)
        ...
        self.action_server_ = actionlib.SimpleActionServer( 'yolov5_action', BoundingBoxesAction, execute_cb = self.image_cb, auto_start = False )
        self.action_server_.start()
        # Spin
        rospy.spin()

    def image_cb(self, goal):
        # Stop intruction
        if self.action_server_.is_preempt_requested():
            # Send action wasblocked by user request
            ...
            try:
                ...
                print(e)
                feedback = BoundingBoxesFeedback(error = True)
...
if __name__ == '__main__':
    rospy.init_node('detector_action')
